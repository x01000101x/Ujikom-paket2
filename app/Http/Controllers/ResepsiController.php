...
class ResepsiController extends Controller
{
    ...
    public function resepsionisFunc(Request $request)
    {
        ...
        $datas = $resepsi::join('kamars', 'kamars.id', 'resepsis.id_kamar')->select('resepsis.id as resepid', 'kamars.id as kamarid', 'resepsis.*', 'kamars.*')->orderBy('resepsis.is_checked', 'asc')->where('resepsis.tamu', 'like', "%" . $keyword . "%")->where('resepsis.booked', 'like', "%" . $date . "%")->where('resepsis.ended', 'like', "%" . $date . "%")->paginate(5);
